...
class Writer:
    ...
    def get_json_output(self):
        ...
        if analyze_packed_result:
            if self.get_analyze_status() == "success":
                formatted = copy.deepcopy(self.__output_dict_vector_result_information)
                for key, vector in formatted.items():
                    for key2, content in vector.items():
    ...
    def save_result_to_txt(self, output_file_path, args):
        ...
        try:
            ...
            print(("<<< Analysis report in TXT is generated: " + os.path.abspath(output_file_path) + " >>>"))
            print("")

            return True
        except IOError as err:
            if DEBUG:
                print("[Error on writing TXT file to disk]")
            return False

    def save_result_to_json(self, output_file_path, args):
        try:
            with open(output_file_path, "w") as f:
                json.dump(self.get_json_output(), f, ensure_ascii=False, indent=4)

            print(("<<< Analysis report in JSON is generated: " + os.path.abspath(output_file_path) + " >>>"))
            print("")

            return True
        except IOError as err:
            if DEBUG:
                print("[Error on writing JSON file to disk]")
            return False
